...
void main(){
    ...
    // float ttcolor = texture(iChannel0, fragCoord/iResolution.xy).x;
    // fragColor = vec4(ttcolor, ttcolor, ttcolor, 1.0);
    // fragColor = vec4(texture(perlinWorley3, vec3(fragCoord/iResolution.xy, 0.0)).x);
}
