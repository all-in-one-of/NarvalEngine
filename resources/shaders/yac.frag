...
float Get3DNoise(vec3 pos) 
{
    ...
    //Another debug
    //return texture(iChannel0, coord).x;
...
float getClouds(vec3 p)
{
    ...
    //this is another debug:
    //return cloudCoord.z * 0.000001;
    //return Get3DNoise(cloudCoord) * 0.01;
    //return  smoothstep(0.55, 0.6, Get3DNoise(cloudCoord));
    ...
    //this is another debug:
    //return  smoothstep(0.55, 0.6, noise);
...
vec3 calculateVolumetricClouds(positionStruct pos, vec3 color, float dither, vec3 sunColor)
{
    ...
    //now this debug:
    //return vec3(getClouds(cloudPosition) * stepLength);
...
void main(){
    ...
    vec2 mouseP = mousePosition;
    mouseP.y = iResolution.y - mouseP.y;
    gatherPositions(pos, fragCoord, mouseP.xy, iResolution.xy);
    ...
    // fragColor = vec4(texture(iChannel0, pos.texcoord).x);
}
